...
output_dir <- "../Desktop/invitro pfas/Testing/Jurkat/"
...
data_48 <- read.csv("../Jurkat/48h analysis.wsp FlowJo table.csv") 
data_72 <- read.csv("../Jurkat/72h analysis.wsp FlowJo table.csv") 
data_96 <- read.csv("../Jurkat/96h analysis.wsp FlowJo table.csv") 
data_48$time <- 48
data_72$time <- 72
data_96$time <- 96
...
cleaned_data <- data %>%
  ...
  rename(
    CFSE_fi = cells.Single.Cells.live.CFSE...Area..FSC...Area.subset...Mean..CFSE...Area.,
...
numeric_columns <- c("CFSE_fi", "live")
cleaned_data <- cleaned_data %>%
  mutate(across(all_of(numeric_columns), ~ as.numeric(gsub(",", ".", .))))

# Pivot the data
cleaned_data <- cleaned_data %>%
  pivot_longer(cols = c("CFSE_fi", "live"), 
               names_to = "Marker", 
               values_to = "Value")
...
# 2) Make sure Dose is numeric (in case it was read as character)
averaged_data <- averaged_data %>%
  mutate(Dose = as.numeric(Dose))

# 3) Build vehicle look-up at Dose = 0
...
# 4) Join back by the natural keys (no need for Vehicle_Control column)
averaged_data <- averaged_data %>%
  left_join(vehicle_lookup, by = c("Replicate", "time", "Chemical", "Marker"))
# Normalize the data based on vehicle control
...
# Create the reciprocal column only for CFSE_fi, and preserve control values (Dose == 0)
normalized_data <- normalized_data %>%
  mutate(
    CFSE_fi_Reciprocal = case_when(
      Marker == "CFSE_fi" & Dose != 0 ~ 1 / Fold_Change,
      TRUE ~ NA_real_  # leave controls and all others blank
    )
  )
...
normalized_data <- normalized_data %>%
  mutate(
    CFSE_fi_Reciprocal = case_when(
      is.na(CFSE_fi_Reciprocal) & Marker == "CFSE_fi" ~ Fold_Change,
      is.na(CFSE_fi_Reciprocal) & Marker == "live" ~ Avg_Value,
      TRUE ~ CFSE_fi_Reciprocal
    )
  )
normalized_data <- normalized_data %>%
  mutate(Sample = str_remove_all(Sample, "PFOS\\+|PFOA\\+"))
# Split by Marker for analysis
...
filtered_datasets <- filtered_datasets[names(filtered_datasets) %in% selected_objects]

# --- SIGNIFICANCE: loop by dataset, time, chemical ---
significance_results <- list()

for (dataset_name in names(filtered_datasets)) {
  dataset <- filtered_datasets[[dataset_name]]

  # ensure numeric dose (avoid "0" vs 0 issues)
  ...
  for (tt in unique_times) {
    ...
    for (chem in unique_chemicals) {
      ...
      # ANOVA per time x chemical
      anova_model <- aov(CFSE_fi_Reciprocal ~ Sample + Error(Replicate/Sample), data = chem_data)
...
# --- Flatten significance to a data frame (carry time) ---
...
# Always put vehicle (0 uM) in group1, treated dose in group2
significance_df <- significance_df %>%
  ...
  mutate(
    dose_num = extract_um(group2)  # numeric version of treated dose
  )

significance_df <- significance_df %>% arrange(time, chemical, dataset_name)

# --- Means/SE per dataset, time, chemical, dose ---
...
for (dataset_name in names(filtered_datasets)) {
  ...
  mean_data <- dataset %>%
    ...
    summarise(
      mean_stat = mean(CFSE_fi_Reciprocal, na.rm = TRUE),
      se_stat   = std.error(CFSE_fi_Reciprocal) * 2.365,
      .groups   = "drop"
    ) %>%
    mutate(
...
# --- Map significance onto filtered_data (match by time & chemical) ---
...
# --- Vehicle ribbon per time x chemical ---
...
replicate_data <- do.call(rbind, filtered_datasets) %>%
  rename(mean_stat = CFSE_fi_Reciprocal) %>%   # <-- rename here
  mutate(
    Dose = as.numeric(Dose),
    log_Treatment = log10(Dose + 1),           # new log10 column
    Chemical = factor(Chemical, levels = chemical_order)
  )
# --- PLOT (facet by time x Chemical) ---
plot_file <- paste0("../Jurkat/", "CFSE_Jurkat_byTime.png")
png(plot_file, width = 2000, height = 1300, res = 300)
...
# --- PLOT (facet by time x Chemical) ---
plot_file <- paste0("../Jurkat/", "CFSE_Jurkat_live.png")
png(plot_file, width = 2000, height = 1300, res = 300)
