...
write.csv(data, "../Mendeley/Figure_3.csv")
```
...
for (chem in unique_chemicals) {
  for (marker in unique_markers) {
    ...
    anova_model <- aov(delta ~ Dose + Error(Replicate/Dose), data = chem_marker_data)
    ...
    max_y <- max(chem_marker_data$delta, na.rm = TRUE)
...
strip.text.y.right = element_blank(),
  ...
  geom_text(data = mean_data[!is.na(mean_data$significance), ],
            ...
            size = 6,
            check_overlap = FALSE)
saveRDS(plot, file = "../pmplot/R/myexample.rds")
print(plot)
dev.off()
```
...
strip.text.y.right = element_blank(),
  ...
  geom_text(data = mean_data[!is.na(mean_data$significance), ],
            ...
            size = 6,
            check_overlap = FALSE)
saveRDS(plot, file = "../pmplot/R/myexample2.rds")
print(plot)
dev.off()
```
#Extra Statitics
...
for (combo_name in names(combo_split)) {
  ...
  if (length(unique(df$Dose)) > 1) {
    cat("Levene's Test:\n")
    lev_result <- car::leveneTest(delta ~ Dose, data = df)
    print(lev_result)
  ...
  # Shapiro-Wilk Test
  if (nrow(df) >= 3 && length(unique(df$delta)) > 2) {
    cat("\nShapiro-Wilk Test:\n")
