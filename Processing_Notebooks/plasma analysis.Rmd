...
library(labelled)
library(corrplot)
  output_dir <- "C:/Users/aloan/OneDrive - HC-SC PHAC-ASPC/Desktop/invitro pfas/Plasma/plots_output/"
...
write.csv(chms, file="../Plasma/chms.csv")
nhanes <- pfas_summary
write.csv(nhanes, file="../Plasma/NHANES.csv")
...
##All chems with LDL
```{r}
data_Plasma <- read.csv("C:/Users/aloan/OneDrive - HC-SC PHAC-ASPC/Desktop/invitro pfas/Plasma/PFAS plasma results.csv")
...
filtered_data <- filtered_data %>%
  mutate(MW = case_when(
    Compound == "PFBA"      ~ 214.04,
    Compound == "PFPeA"     ~ 264.04,
    Compound == "PFHxA"     ~ 314.03,
    Compound == "PFHpA"     ~ 364.02,
    Compound == "PFOA"      ~ 414.01,
    Compound == "PFNA"      ~ 463.99,
    Compound == "PFDA"      ~ 513.98,
    Compound == "PFUdA"     ~ 563.97,
    Compound == "PFDoA"     ~ 613.96,
    Compound == "PFTrDA"    ~ 663.94,
    Compound == "PFTeDA"    ~ 713.93,
    Compound == "PFBS"      ~ 299.09,
    Compound == "PFPeS"     ~ 349.09,
    Compound == "PFHxS"     ~ 399.08,
    Compound == "PFHpS"     ~ 449.07,
    Compound == "PFOS"      ~ 499.06,
    Compound == "PFNS"      ~ 549.05,
    Compound == "PFDS"      ~ 599.04,
    Compound == "X4_2.FTS"  ~ 364.10,
    Compound == "X6_2_FTS"  ~ 464.10,
    Compound == "X8_2.FTS"  ~ 564.10,
    TRUE ~ NA_real_
  ))

#ng/ml to um
filtered_data$uM <- filtered_data$Value * 1000/ filtered_data$MW 
...
plot <- ggplot(filtered_data, aes(x = Compound, y = uM, color = Sample, group = Sample)) +
  stat_summary(
    aes(fill = Sample),
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, max(filtered_data$uM, na.rm = TRUE) * 1.2)) + 
  labs(y = "uM", x = "Compound", title = "PFAS Levels in Plasma with MDL")
...
filtered_data %>%
  group_by(Compound) %>%
  summarise(mean_uM = mean(uM, na.rm = TRUE))

write.csv(filtered_data, "../Mendeley/Figure_1.csv")
```
