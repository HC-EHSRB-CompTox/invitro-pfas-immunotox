...
cleaned_data_pha <- cleaned_data %>% filter(str_detect(Treatment, "PHA"))
selected_objects <- c("Sex", "Treatment","Group" , "Chemical", "CD3_fi", "CD4_fi", "CD8_fi")

filtered_datasets <- cleaned_data_pha[names(cleaned_data_pha) %in% selected_objects]
write.csv(filtered_datasets, "C:/Users/aloan/OneDrive - HC-SC PHAC-ASPC/Desktop/invitro pfas/Flow cyto (PBMCs)/Combined_flow_pha_fi.csv")

# Normalize
 normalize_by_vehicle <- function(df) {
  numeric_columns <- c( "CD3_fi", "CD4_fi", "CD8_fi", "Ms_fi", "dead_fi")
...
columns <- c("live", "CD3", "CD4", "CD8", "CD3_fi", "CD4_fi", "CD8_fi","CD3_divided", "CD4_divided", "CD8_divided", "CD4_fi_reciprocal", "CD3_fi_reciprocal", "CD8_fi_reciprocal", "Ms", "Ms_fi_reciprocal", "Ms_divided", "dead_fi_reciprocal", "count")
...
calculate_delta <- function(df, vehicle_name) {
  df %>%
    ...
    mutate(delta = Statistic - Statistic[Treatment == vehicle_name]) %>%
    ungroup()
}
# Apply to PHA datasets
pha_datasets <- lapply(pha_datasets, calculate_delta, vehicle_name = "PHA + Vehicle")

# Apply to LPS datasets
#LPS is missing some vehicles becasue of my count filtering

lps_datasets<- lapply(lps_datasets, calculate_delta, vehicle_name = "LPS + Vehicle")

datasets <- c(pha_datasets, lps_datasets)
combined_df <- bind_rows(datasets, .id = "source")

write.csv(combined_df, "../Mendeley/Figure_2.csv", row.names = FALSE)
```
...
for (dataset_name in names(filtered_datasets)) {
  ...
  for (chem in unique_chemicals) {
    chem_data <- dataset %>% filter(Chemical == chem)
    anova_model <- aov(delta ~ Treatment + Error(Group/Treatment), data = chem_data)
    anova_summary <- summary(anova_model)
cat("\n\n=== ANOVA results for dataset:", dataset_name, "chemical:", chem, "===\n")
    print(anova_summary)
    treatment_p <- tryCatch({
...
levene_results <- lapply(datasets, function(dataset) {
  leveneTest(delta ~ Treatment, data = dataset)
})
...
shapiro_results <- lapply(datasets, function(dataset) {
  shapiro.test(dataset$delta)
})

print(shapiro_results)

lapply(datasets, function(dataset) {
  qqnorm(dataset$delta)
  qqline(dataset$delta)
})
