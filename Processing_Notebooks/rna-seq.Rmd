...
#Revision
```{r}
# b cell indentiy markers 

padj_thresh <- 0.05
log2FC_thresh <- 0.5
highlight_genes <- c(
  "CD19","CD20", "CD21","CD22","CD23","CD24","CD27", "CD38", "IGHM")

# Combine male + female — filtered to highlight genes only
deg_combined <- bind_rows(
  deg_f %>% mutate(Sex="Female"),
  deg_m %>% mutate(Sex="Male")
) %>%
  filter(Gene_Symbol %in% highlight_genes) %>%     # <-- only keep labelled genes
  mutate(Significant = case_when(
    padj < padj_thresh & log2FoldChange > log2FC_thresh  ~ "Up",
    padj < padj_thresh & log2FoldChange < -log2FC_thresh ~ "Down",
    TRUE ~ "NS"
  ...
  mutate(contrast_clean = str_replace_all(contrast, "_[FM](?=\\b)", "")) %>%
  mutate(highlight = Gene_Symbol)   # all rows are highlight genes now

deg_combined$contrast_clean <- factor(
  deg_combined$contrast_clean,
  levels = c("PFOS vs Control", "PFOA vs Control", "PFNA vs Control")
)

volcano<- ggplot(deg_combined, aes(x = log2FoldChange, y = -log10(padj),
                          fill = Significant, shape = Sex)) +
  geom_point(alpha = 0.6, size = 2, color = "black", stroke = 0.5) +
  scale_fill_manual(values = c("Up" = "maroon", "Down" = "navy", "NS" = "grey")) +
  scale_shape_manual(values = c("Female" = 21, "Male" = 24)) +
  scale_color_manual(values = c("Up" = "maroon", "Down" = "navy", "NS" = "grey"),  # <-- add this
                     guide = "none") +                                               # hide from legend
  ggrepel::geom_text_repel(
    aes(label = highlight, color = Significant),   # <-- add color = Significant here
    size = 4,
    max.overlaps = 25,
    force = 10,
    box.padding   = unit(0.75, "lines"),
  ...
  labs(title = "Combined Male & Female Volcano Plots by Contrast",
       ...
       y = "-log10(adjusted p-value)") +
  facet_wrap(~contrast_clean, scales = "free") +
  coord_cartesian(xlim = c(-4, 4), ylim = c(-5, 35))
print(volcano)

ggsave("C:/Users/aloan/OneDrive - HC-SC PHAC-ASPC/Desktop/invitro pfas/rna -seq/plots/_revisions_volcano.png",
       plot = volcano, bg = "white", height = 5, width = 10)
