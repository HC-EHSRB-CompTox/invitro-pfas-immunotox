...
combined_bmd_filtered <- combined_bmd %>%
  ...
  mutate(
    ...
    filter_reason = case_when(
      hit < 0.9 ~ "low hit call",
      is.na(BMDL) ~ "missing BMDL",
      (BMDU / BMDL) >= 50 ~ "high BMDU/BMDL ratio",
      BMD < minDose | BMD > maxDose ~ "outside dose range",
      TRUE ~ NA_character_
    ),

    # Only keep BMDs if:
    #  - no filter reason
    #  - OR only issue is outside dose range
    BMD = ifelse(
      is.na(filter_reason) | filter_reason == "outside dose range",
      BMD, NA
    ),
    BMDL = ifelse(
      is.na(filter_reason) | filter_reason == "outside dose range",
...
print(p)
ggsave("/plots/BMD_Comparison_DotWhisker_filtered.png", p, width = 10, height = 6)
# Filter for PFOS and TNF.a
filtered_data <- combined_bmd_filtered %>%
  filter(Chemical == "PFOS", Marker == "TNF.a")


combined_bmd_filtered <- combined_bmd_filtered %>%
  left_join(dose_ranges, by = c("Chemical", "Source")) %>%
  mutate(
    # APPLY FILTERING 
    BMD = ifelse(is.na(filter_reason), BMD, NA),
    BMDL = ifelse(is.na(filter_reason), BMDL, NA),
    BMDU = ifelse(is.na(filter_reason), BMDU, NA)
  )
# Then plot
p <- ggplot(combined_bmd_filtered, aes(x = BMD, y = Marker, color = Source)) +
  geom_point(position = position_dodge(width = 0.7), size = 3) +
...
ggsave("/plots/BMD_Comparison_DotWhisker_filtered2.png", p, width = 8, height = 4)
```
##DRC
```{r}
...
p
ggsave("/plots/drc2.png", p, width = 8, height = 4)
```
